    ...
    return(<div className="w-full px-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {productData.map(({id,
           ...
           <div key={id} className="w-full bg-white text-black p-4 border-gray-300 rounded-lg group overflow-hidden">
            ...
            <p className="flex items-center"><span className="text-amazon_blue font-bold"><FormattedPrice amount={price*100}/></span></p>
            <p className="text-xs test-gray-500 text-justify">{description.substring(0,200)}</p>
            ...
            )}className="h-10 font=medium bg-amazon_blue text-white round-md hover:bg-amazon_yellow hover:text-black duration-300 mt-2">Add To Cart</button>
           </div>
           </div>
        ))}
    </div>);
